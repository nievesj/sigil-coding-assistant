...
| 2026-08-02 | — | **M7 gate removed (bug fix):** The skill palette now opens on bare `$` (showing all skills, unfiltered), matching the slash palette's behavior on bare `/`. Previously the gate required `text.length > 1` and a valid skill-name-start char at `text[1]`, so typing `$` alone showed nothing — the user concluded the feature was broken. The M7 rationale ("prevent `$50` from opening the palette") was discarded: showing an empty/no-match palette for `$50` is acceptable and consistent with how the slash palette handles unmatched `/` queries. Gate logic extracted into pure `detectSkillTrigger()` (mirrors `detectMentionTrigger`) with `SkillTriggerTest`. `$50` and `$PATH` now both open the palette (filtered by "50" / "PATH"). `$$` escape and newline guard retained. |
| 2026-08-02 | — | **Plugin-ID gate removed + Codex path removed (bug fix):** The IDE skill path was gated on `PluginManager.isPluginInstalled(PluginId.getId("com.intellij.ai.assistant"))`, but the AI Assistant plugin ID changed to `com.intellij.ml.llm` in IntelliJ 2026.2 (verified via `META-INF/plugin.xml` in `ml-llm.jar`). The stale ID caused `isPluginInstalled` to return `false`, so `detectSkillPaths()` returned an empty list, `writeSkillPaths(emptyList())` wrote no `skills.paths` to `opencode.json`, and OpenCode never discovered the IDE skills — the whole point of the feature. The gate is now **directory existence** only: if `aia/agents/.agents/skills` exists, the AI Assistant created skills there and they should be bridged regardless of plugin load state. This is robust against future plugin-ID renames. The Codex Global path (`~/.codex/skills`) was removed entirely — Codex scoping is not used and may never be, so carrying a dead path and a second `isPluginManagedPath` suffix was unnecessary surface area. `isPluginManagedPath` now matches only `/aia/agents/.agents/skills`. `detectSkillPathsPure` signature simplified (dropped `isAiAssistantInstalled` + `userHome` params). Added diagnostic logging (`[ACP] JetBrainsSkillBridge: detected N skill path(s)`). Tests updated: `JetBrainsSkillBridgeTest` rewritten for the new API; `McpConfigWriterSkillPathsTest` scenario 6b updated (codex path is now user-managed, not evicted). |